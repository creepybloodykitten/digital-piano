...
module piano_voice(
    ...
    reg [31:0] phase_acc = 0; // То, что мы слышим (основная нота)
    reg [31:0] phase_mod = 0; // То, что создает тембр (модулятор)

    wire [15:0] freq_mod = freq; // Частота модулятора в 2 раза выше основной
    ...
    always @(posedge clk_lrck) begin
        if (freq == 0) begin
            ...
            phase_acc <= phase_acc + {freq, 16'b0};
            phase_mod <= phase_mod + {freq_mod, 16'b0};
        end
    end

    // 2. Читаем синус для модулятора
    wire signed [15:0] mod_wave;
    wave_rom rom_mod (
        .address(phase_mod[31:22]),
        .clock(clk_lrck),
        .q(mod_wave)
    );

    // 3. Умножаем модулятор на громкость (envelope) 
    // Чем громче звук - тем сильнее искажается фаза (больше гармоник)
    wire signed [32:0] phase_offset_calc = $signed(mod_wave) * $signed({1'b0, envelope});
    wire [9:0] phase_offset = phase_offset_calc >>> 22; // Берем нужные биты для сдвига

    // 4. Читаем основной синус, прибавив искажение к адресу!
    wire [9:0] rom_addr = phase_acc[31:22] + phase_offset;
    wire signed [15:0] raw_wave;
